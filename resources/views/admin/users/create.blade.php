<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('སྤྱོད་མི་གསར་བཟོ།') }} (Create New User)
        </h2>
    </x-slot>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('admin.users.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            <label for="password" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('གསང་ཚིག') }} (Password)</label>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="role" id="role"
                                class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 rounded-md">
                                <option value="admin" {{ old('role') === 'admin' ? 'selected' : '' }}>{{ __('འཛིན་སྐྱོང་པ།') }} (Admin)</option>
                                <option value="chief_editor" {{ old('role') === 'chief_editor' ? 'selected' : '' }}>{{ __('རྩོམ་སྒྲིག་འགན་འཛིན།') }} (Chief Editor)</option>
                                <option value="editor" {{ old('role') === 'editor' ? 'selected' : '' }}>{{ __('རྩོམ་སྒྲིག་པ།') }} (Editor)</option>
                                <option value="benchmark_editor" {{ old('role') === 'benchmark_editor' ? 'selected' : '' }}>{{ __('ཚད་འཇལ་རྩོམ་སྒྲིག་པ།') }} (Benchmark Editor)</option>
                                <option value="reviewer" {{ old('role') === 'reviewer' ? 'selected' : '' }}>{{ __('ཞུ་དག་པ།') }} (Reviewer)</option>
                            </select>
                            @error('role')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                                    ...
                                    @foreach($categories as $category)
                                        <div class="flex items-center">
                                            <input type="checkbox" name="allowed_categories[]" id="category-{{ $loop->index }}" value="{{ $category->name }}"
                                                {{ (is_array(old('allowed_categories')) && in_array($category->name, old('allowed_categories'))) ? 'checked' : '' }}
                                                class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded category-checkbox">
                                            <label for="category-{{ $loop->index }}" class="ml-2 block text-sm text-gray-900 dark:text-gray-300 tibetan-font">
